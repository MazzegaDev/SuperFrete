...
function calc(){
    ...
    if(!customerName.trim() || !customerNum.trim() || isNaN(km) || isNaN(oilPrice) || isNaN(spentOil) || isNaN(toll)){
        errorDiv.innerText = 'Por favor, preencha corretamente todos os campos.'
    }else{
        let profit = calcProfit(spentOil, oilPrice, toll, km);
        renderTable(customerName, customerNum, km, oilPrice, spentOil, toll, table, profit);
    }
}

function calcProfit(spentOil, oilPrice, toll,km){
    const fixedNumber = 3;
    let oilPerKm = km / spentOil;
    let pricePerKm = oilPerKm * fixedNumber;
    let price = pricePerKm * oilPrice;
    let finalProfit = price - toll
 
    
    return finalProfit.toFixed(2);
}

function renderTable(customerName, customerNum, km, oilPrice, spentOil, toll, table, profit){
    shipping.push({cName: customerName, cNum: customerNum, km: km, oilPrice: oilPrice, spentOil: spentOil, toll: toll, profit: profit});
